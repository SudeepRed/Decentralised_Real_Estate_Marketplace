      ...
      {this.props.listings.map((property,key) =>{
        ...
        <div className="details">
            <div className="content">
                ...
                <div className = "Price">{window.web3.utils.fromWei(property.priceForRent.toString(), 'Ether')} Eth</div>
                <button className = "buyNow" 
                               ><Link to={`/settenant`}>Set Tenant</Link></button>
                <div>
                  {
                    (this.props.tenant == this.props.account)
                    ? <button className = "buyNow">
                      <Link to={`/payrent`} state={{ price: window.web3.utils.fromWei(property.priceForRent.toString(), 'Ether') }}>Pay Rent</Link>
                    </button>
                    : null
                  }
                </div>
